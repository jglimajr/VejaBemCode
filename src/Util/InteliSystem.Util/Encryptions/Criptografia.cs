    ...
    public class Criptografia
    {
		...
		public static string GerarChave(string ibase = "", short tamanho = 24)
		{
			...
			for (int i = 0; i <= tamanho; i++) {
				...
				chave += Byte.Parse(i.ToString()).ToString("X");
			}
		...
		public string BlowFishEncript(string value)
		{
			var blow = new BlowFish(Encoding.UTF8.GetBytes(Chave));
			var Retorno = blow.Encrypt_CBC(value);
			return Retorno;
		}

		public string BlowFishDecript(string value)
		{

			var blow = new BlowFish(Encoding.UTF8.GetBytes(Chave));
			var Retorno = blow.Decrypt_CBC(value);
			return Retorno;
		}
		...
		//private string GetChave(string nome)
		//{
		//	return "";
		//}
		/*
        public static void GravarChave(string nome)
        {
            if(des == null) {
                des = new TripleDESCryptoServiceProvider();
            }
            var cp = new CspParameters();
            cp.KeyContainerName = nome;
            cp.KeyNumber = 1;
            des.GenerateKey();
            //cp.CryptoKeySecurity = new System.Security.AccessControl.CryptoKeySecurity();
            //cp.CryptoKeySecurity.

            var keycontainer = new DSACryptoServiceProvider(cp);
            keycontainer.

            var teste = keycontainer.ToXmlString(true);
